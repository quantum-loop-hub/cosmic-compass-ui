...
serve(async (req) => {
  ...
  try {
    ...
    const { text, language = 'hi' }: TTSRequest = await req.json();
    ...
    const voiceId = VOICES[language] || VOICES.hi;

    console.log(`Generating TTS for ${truncatedText.length} chars in ${language} for user ${userId}`);
